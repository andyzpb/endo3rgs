...
class PointCloudViewer:
    def __init__(
        ...
        # if gt_global_pts is not None:
        self.gt_global_pts = gt_global_pts
        # else:
        #     self.gt_global_pts = None
        ...
        # self.add_gt_point_cloud()

    def add_gt_point_cloud(self):
        """Add the GT point cloud to the Viser server."""
        if self.gt_global_pts is None:
            return
        # Ensure gt_global_pts is a NumPy array
        points = np.asarray(self.gt_global_pts)
        if points.ndim != 2 or points.shape[1] != 3:
            raise ValueError(f"gt_global_pts must be Nx3, got shape {points.shape}")
        
        # Set default color (red) for GT points
        colors = np.full((points.shape[0], 3), [1.0, 0.0, 0.0], dtype=np.float32)
        
        # Add GT point cloud to the server
        self.gt_handle = self.server.add_point_cloud(
            name="/gt_point_cloud",
            points=points,
            colors=colors,
    ...
    def add_camera(self, step):
        ...
        # Extract pose and intrinsic parameters
        pose = cam["pose"][step]  # Assuming pose is a 4x4 transformation matrix
        intrinsic = cam["intrinsic"]  # Assuming intrinsic is a 3x3 matrix

        # Decompose the pose into rotation (R) and translation (t)
        R = pose[:3, :3]  # Extract the 3x3 rotation matrix
        t = pose[:3, 3]   # Extract the 3x1 translation vector

        # Extract focal length and principal point from intrinsic matrix
        focal = intrinsic[0, 0]  # Assuming fx is the focal length
        pp = (intrinsic[0, 2], intrinsic[1, 2])  # Principal point (cx, cy)
