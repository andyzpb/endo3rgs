...
def get_matches(flow_fw, h, w):
    flow = torch.from_numpy(flow_fw[0]).float().permute(1,2,0)
    # flow = self.record_data['flows_fw'][timestep_1].permute(1,2,0)
...
def get_fundamental_matrix(Rt_1, Rt_2, intrinsic):
    ...
    # intrinsic = torch.from_numpy(self.record_data['intrinsic']).unsqueeze(0).float()
...
def adaptive_thresholding(tensor, factor=0.5):
    """
    ...
    threshold = median#mean_value - factor * std_value#- factor * std_value
    # print(mean_value, std_value, threshold, median)
...
def compute_epipolar_mask(flow_fw, Rt_1, Rt_2, h, w, intrinsic):
    ...
    points1 = match[:,:2,:].permute(0, 2, 1).cpu()
    points2 = match[:,2:,:].permute(0, 2, 1).cpu()
